...
PEREGON_ALIASES: dict[str, str] = {
    ...
    "магнетит и шон": "Магнетиты — Шонгуй",
    "магнитит и шон": "Магнетиты — Шонгуй",
    "магнетиты и шон": "Магнетиты — Шонгуй",
    "магнитит и шонгуй": "Магнетиты — Шонгуй",
    "магнетиты и шонгуй": "Магнетиты — Шонгуй",
    "никиты — шонгу": "Магнетиты — Шонгуй",
    "никиты — шонгуй": "Магнетиты — Шонгуй",
    "никиты — шомгу": "Магнетиты — Шонгуй",
    "магнетиты — шонгу": "Магнетиты — Шонгуй",
...
def _fix_asr_typos(value: str) -> str:
    """Типичные ошибки ASR для станции Комсомольск-Мурманский и перегонов."""
    s = value
    s = re.sub(r"шомгу\b", "шонгуй", s, flags=re.IGNORECASE)
    s = re.sub(r"шонгу\b", "шонгуй", s, flags=re.IGNORECASE)
    s = re.sub(r"\bникиты\b", "магнетиты", s, flags=re.IGNORECASE)
    s = re.sub(
...
def normalize_peregon(value: str | None) -> str | None:
    ...
    fixed = value.strip()
    # ASR иногда «липнет» км к названию: «магнитит и шон 2419»
    fixed = re.sub(r"\s+\d+$", "", fixed).strip()
    fixed = _fix_asr_typos(fixed)
